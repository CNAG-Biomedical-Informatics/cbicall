...
CBIcall: CNAG Biomedical Informatics Framework for Variant Calling on Illumina sequencing data.
...
- Input Files
    ...
    A folder containing Paired-End FASTQ files (e.g., `CNAG999_exome/CNAG99901P_ex//*{R1,R2}*fastq.gz`).
...
CBIcall is optimized for multi-core Linux desktop, workstation, or server environments.
    ...
    * Works on amd64 and arm64 architectures (including Apple Silicon / M-based Macs).
    * Ideally a Debian-based distribution (Ubuntu or Mint), but other distributions (e.g., CentOS, OpenSUSE) should also work (untested).
    * ≥ 8 GB RAM.
    * ≥ 4 CPU cores (Intel i7 or Xeon preferred).
    * ≥ 250 GB HDD space.

## Additional requirements (source-based installation only)

The following software is **only required if you install CBIcall from source**:

    * Python ≥ 3.8
    * Java 8 (install via `sudo apt install openjdk-8-jdk`)
    * Snakemake (install via `pip3 install -r requirements.txt`)
