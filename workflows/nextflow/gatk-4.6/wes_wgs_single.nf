...
def REFGZ        = expandPlaceholders(RESOURCE_CFG.refgz.toString(), [bundle: BUNDLE] + lvl2)
...
def KG_INDELS    = expandPlaceholders(RESOURCE_CFG.kg_indels.toString(), [bundle: BUNDLE] + lvl2)
...
def COV      = params.coverage_script ? params.coverage_script.toString() : file("${projectDir}/coverage.sh").toString()
def VCF2SEX  = params.vcf2sex_script ? params.vcf2sex_script.toString() : file("${projectDir}/vcf2sex.sh").toString()
...
process COVERAGE_STATS {
    ...
    input:
    tuple path(raw_bam), path(raw_bai), path(recal_bam), path(recal_bai)
...
process VCF_HASH {
    publishDir STATSDIR, mode: 'copy', pattern: "${ID}.vcf.sha256.txt"
    publishDir LOGDIR,   mode: 'copy', pattern: '*.log'
    ...
    bash ${q(VCF2HASH)} ${q(qc_vcf)} > ${q("${ID}.vcf.sha256.txt")} 2>> ${q("${ID}.10_vcf_hash.log")}
    """
}
...
workflow {
    ...
    coverage_input = dedup[0].combine(dedup[1]).combine(bqsr[1]).combine(bqsr[2])
