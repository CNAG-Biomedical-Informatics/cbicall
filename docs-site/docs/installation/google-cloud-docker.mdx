...
YAML, workflow registry, and resource bundle resolve and run outside your local
...
because the external resource bundle is the large part of the test.
...
Download, assemble, verify, and extract the external resource bundle:

```bash
cbicall_docker python3 scripts/01_download_external_data.py \
  --outdir /cbicall-data \
...
```bash
grep -E '"status"|"pipeline_version"|"resource_bundle"|"fingerprint"|"workflow_log"' \
...
> and resource bundle as the local test. CBIcall resolved the same workflow
> implementation and resource bundle identity, and the generated VCF matched the
