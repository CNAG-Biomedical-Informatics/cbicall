...
| `cbicall_external_nextflow.config` | Nextflow config generated by CBIcall to cap process CPU requests from `-t/--threads`. |
...
value. The generated Nextflow config applies the same value through
...
AMD64. For macOS/Apple Silicon smoke testing, `nf-core/demo` can also be run
with `nextflow_profile: test,conda` to avoid Docker architecture emulation.
