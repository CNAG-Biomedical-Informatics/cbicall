...
| `run-report.html` | Human-readable rendering of `run-report.json` for browsing a completed run without reading JSON directly. It links the main run evidence and shows software-version evidence when available. |
...
Use `outputs.file_inventory.sha256` to check whether two run directories contain the same relative file layout. This is a manifest hash of file paths, not a content hash. `outputs.file_inventory.total_bytes` records the total size of files included in that inventory. WES/WGS single-sample runs also include parsed VCF hash reports under `outputs.vcf_hash_reports` when `03_stats/*.vcf.sha256.txt` is present.
