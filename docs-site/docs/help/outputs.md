...
Bundled `cbicall-core` pipelines (`wes`, `wgs`, and `mit`) create the run directory
...
<details>
<summary>Audit files and run reports</summary>
...
![Screenshot of the bundled CBIcall run-report HTML overview tab, showing the run summary and audit sections.](/img/native-run-report-overview.png)

![Screenshot of the bundled CBIcall run-report HTML evidence tab, showing workflow fingerprints, resource identity, runtime versions, and linked audit artifacts.](/img/native-run-report-evidence.png)

![Screenshot of the bundled CBIcall run-report HTML outputs tab, showing canonical files, output inventory, and file-size summaries.](/img/native-run-report-outputs.png)
...
`outputs.file_inventory.sha256` identifies the relative file layout, not file
contents. `outputs.file_inventory.total_bytes` records the inventory size.
Use [Run Comparison](run-comparison) to compare these fields and final-output
fingerprints across two or more runs.

</details>

<details>
<summary>MultiQC custom content</summary>
...
CBIcall-specific MultiQC plugin is required. Install the optional renderer with
`python3 -m pip install "cbicall[multiqc]"` when needed.
...
<details>
<summary>External nf-core output layout</summary>
...
The sections below list the files users normally inspect for each analysis.
