...
If the new pipeline fits the existing concepts of `pipeline`, `mode`, `workflow_backend`, `gatk_version`, `input_dir`, and `sample_map`, start with workflow scripts and the registry only.
:::
...
file to launch when a parameters YAML selects values such as `workflow_backend`,
`gatk_version` or `workflow_provider`, `pipeline`, `mode`, and `pipeline_version` in the parameters
YAML.
...
Add the pipeline under the correct backend and version.
...
:::info[Registry paths]
Registry paths are relative to the backend/version directory. For Bash GATK 4.6, `mypipe_single.sh` resolves below `workflows/bash/gatk-4.6/`.
:::
...
| --- | --- |
| Registry | The workflow appears under the correct backend/version/pipeline/mode. |
...
- [ ] Pick backend, version, pipeline name, and mode.
