...
If the new pipeline fits the existing concepts of `pipeline`, `mode`, `workflow_backend`, `software_stack`, `input_dir`, and `sample_map`, start with workflow scripts and the registry only.
:::
...
`software_stack` or `workflow_provider`, `pipeline`, `mode`, and `pipeline_version` in the parameters
YAML.
...
| --- | --- |
| Registry | The workflow appears under the correct backend/software-stack/pipeline/mode. |
