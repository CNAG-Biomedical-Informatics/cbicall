...
| Software stack | `gatk-3.5`, `gatk-4.6`, `nf-core` | Determines the workflow subdirectory, shared helper files, or external workflow source. |
...
`workflow_provider`, `software_stack`, `pipeline`, `mode`, and `pipeline_version`.
