...
CBIcall records three complementary VCF checks because different questions need
different levels of sensitivity.

| Fingerprint | Fields hashed | Example of change detected | Typical interpretation |
| --- | --- | --- | --- |
| `sample order` | Ordered sample names from columns 10 onward in the VCF `#CHROM` header. | A sample is renamed, added, removed, or reordered. | The compared VCFs do not describe the same ordered sample set. Check this before interpreting call-level agreement. |
...
The sample-order check was added without changing the call-level or strict-record
hash definitions. The cross-environment hashes reported above therefore remain
directly comparable with the original analysis.
