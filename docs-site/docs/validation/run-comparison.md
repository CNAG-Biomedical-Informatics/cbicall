...
overview, comparison matrix, detailed rows, and raw text report.

The **Matrix** tab is useful for cross-environment audits. Rows correspond to
framework, workflow, resource, execution, and output fields; columns correspond
to compared runs; and each cell shows whether that run is `same`, `different`,
`missing`, `note`, or `not available` relative to the baseline. Use `--alias`
to make opaque run directories readable in both text and HTML reports.
